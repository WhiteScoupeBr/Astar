...
class No{
private:
int i,j,pai_i,pai_j,position,h,f,valor;
float g;

public:
  No(int i = -1, int j = -1, int pai_i = -1, int pai_j = -1, int position = SOUTHEAST, float g = 9999, int h = 99999, int f=9999){
    this.i=i;
    this.j=j;
    this.pai_i=pai_j;
    this.pai_j=pai_j;
...
public:
  int calculaH(int i_incio,int j_inicio,int i_dest, int j_dest){
     return max(abs(i_incio - i_dest), abs(j_inicio - j_dest));
  };

  void busca(int i_incio,int j_inicio,int i_dest, int j_dest){

    No *nodeInicio= new No(i_incio,j_inicio,-1,-1,EAST);
    No *nodeFinal= new No(i_dest,j_dest,-1,-1);
  ...
  void calculaValores(No *pai){
    ...
    if(getTarget(i,j){
      ...
      printf("Encontrou %d %d", node->getX(), node->getY());
      listaFechada.push_back(pai);
      removeOpenList(pai);
      for (int i = 0; i < listaFechada.size(); i++)
      {
        printf("\nCaminho: %d %d", listaFechada[i]->getX(), listaFechada[i]->getY());
      }
      return;
    }
  ...
  int getNodePositionInOpenList(Node *node)
  {
    for (int i = 0; i < listaAberta.size(); i++)
    {
      if (node == listaAberta[i])
  ...
  void removeNodeOpenList(Node *node)
  {
    listaAberta.erase(listaAberta.begin() + getNodePositionInOpenList(node) - 1);
  }

  bool isNodeInCloseList(Node *node)
  {
    for (int i = 0; i < listaFechada.size(); i++)
    {
      if (node->getX() == listaFechada[i]->getX() && node->getY() == listaFechada[i]->getY())
      {
        return true;
      }
    }
